...
function Passenger({ snapshot, activeRisk }: { snapshot: CockpitSnapshotV1 | null; activeRisk?: RiskEventV1 }) {
  ...
  return <div className="passenger-layout"><div className={`media-card ${suppressed ? 'is-suppressed' : ''}`}><PauseCircle size={36} /><p>{suppressed ? '媒体已因驾驶风险抑制' : '媒体与旅程协作'}</p><b>{suppressed ? '请协助驾驶员处置告警' : playing ? '当前播放 · 可由副驾控制' : '媒体已暂停'}</b><div className="command-stack"><button className="secondary-button" disabled={pending || suppressed} onClick={() => void send('set_media_state', { state: playing ? 'paused' : 'playing' })}>{playing ? '暂停媒体' : '播放媒体'}</button><input value={suggestion} maxLength={200} onChange={(event) => setSuggestion(event.target.value)} aria-label="旅程建议" /><button className="secondary-button" disabled={pending || !suggestion.trim()} onClick={() => void send('submit_trip_suggestion', { suggestion })}>发送旅程建议</button></div></div><div className="privacy-card"><ShieldAlert /><div><p>隐私模式</p><b>{snapshot?.passenger.privacyEnabled ? '副驾内容不投射至驾驶端' : '副驾内容可共享至中控'}</b></div><button className="secondary-button" disabled={pending} onClick={() => void send('set_cabin_control', { privacyEnabled: !snapshot?.passenger.privacyEnabled })}>{snapshot?.passenger.privacyEnabled ? '关闭隐私' : '开启隐私'}</button></div><VisionCard risk={activeRisk} />{error && <p className="command-error" role="alert">{error}</p>}</div>
}

function ReadOnlyCenterPreview({ snapshot, activeRisk }: { snapshot: CockpitSnapshotV1 | null; activeRisk?: RiskEventV1 }) {
  return <div className="center-layout"><div className="map-surface"><MapPin /><p>{snapshot?.navigation.destinationName ?? '未设置目的地'}</p><span>{snapshot?.navigation.provider === 'local_fallback' ? '本地路线 · 降级' : '路线已同步至主仪表与 HUD'}</span></div><div className="center-side"><DataTile icon={<Gauge />} label="驾驶模式" value={snapshot?.vehicle.driveMode ?? '—'} /><VisionCard risk={activeRisk} />{activeRisk && <RiskCard risk={activeRisk} />}</div></div>
}

function ReadOnlyPassengerPreview({ snapshot, activeRisk }: { snapshot: CockpitSnapshotV1 | null; activeRisk?: RiskEventV1 }) {
  const suppressed = isMediaSafetySuppressed(snapshot?.risks ?? [])
  const playing = snapshot?.passenger.mediaState === 'playing'
  return <div className="passenger-layout"><div className={'media-card ' + (suppressed ? 'is-suppressed' : '')}><PauseCircle size={36} /><p>{suppressed ? '媒体已因驾驶风险抑制' : '媒体与旅程协作'}</p><b>{suppressed ? '请协助驾驶员处置告警' : playing ? '当前播放 · 可由副驾控制' : '媒体已暂停'}</b></div><div className="privacy-card"><ShieldAlert /><div><p>隐私模式</p><b>{snapshot?.passenger.privacyEnabled ? '副驾内容不投射至驾驶端' : '副驾内容可共享至中控'}</b></div></div><VisionCard risk={activeRisk} /></div>
}
function Overview({ snapshot, activeRisk, connection }: { snapshot: CockpitSnapshotV1 | null; activeRisk?: RiskEventV1; connection: ConnectionState }) {
  return <div className="overview-grid">{(['cluster', 'hud', 'center', 'passenger'] as const).map((endpoint) => <div className="screen-preview" key={endpoint}><span>{endpointNames[endpoint]}</span>{endpoint === 'center' ? <ReadOnlyCenterPreview snapshot={snapshot} activeRisk={activeRisk} /> : endpoint === 'passenger' ? <ReadOnlyPassengerPreview snapshot={snapshot} activeRisk={activeRisk} /> : <EndpointCanvas endpoint={endpoint} snapshot={snapshot} activeRisk={activeRisk} />}</div>)}<p className="overview-footnote">{connection === 'connected' ? '四屏均消费同一 FastAPI snapshot。' : '等待权威 snapshot；不会用客户端推测值替代。'}</p></div>
}
