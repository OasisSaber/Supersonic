    ...
    <div className="sp-overview-layout">
      <section className="sp-overview-summary" aria-label="多屏权威状态摘要">
        <div>
          ...
          <span>连接端点</span>
          <strong className="sp-tabular">{online}/6</strong>
        </div>
        <div>
      ...
      <div className="sp-overview-grid">
        ...
        <PreviewCard endpoint="center">
          ...
          <span>{snapshot?.navigation.status ?? 'idle'} · read only</span>
        </PreviewCard>

        <PreviewCard endpoint="passenger">
          <strong>{snapshot?.passenger.mediaState ?? 'paused'}</strong>
          <span>{snapshot?.passenger.privacyEnabled ? '隐私模式开启' : '允许共享旅程内容'}</span>
          <div className="sp-preview-inline">建议 {snapshot?.passenger.tripSuggestions.length ?? 0} 条</div>
        </PreviewCard>
      </div>
